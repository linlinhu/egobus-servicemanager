...
Ext.apply(Ext.EventManager, function() {
	var c, j, e, b, a = Ext.lib.Dom, k = Ext.lib.Event, i = /^(?:scope|delay|buffer|single|stopEvent|preventDefault|stopPropagation|normalized|args|delegate)$/, h = 0, g = 0, d = Ext.isWebKit
...
Ext.Element.addMethods(function() {
			...
			return {
				...
				replaceWith : function(e) {
					var g = this, d = Ext.Element;
					if (e.nodeType || e.dom || typeof e == "string") {
						e = c(e);
						g.dom.parentNode.insertBefore(e, g.dom)
					} else {
						e = b.insertBefore(g.dom, e)
					}
					delete d.cache[g.id];
					Ext.removeNode(g.dom);
					g.id = Ext.id(g.dom = e);
					return d.cache[g.id] = g
				},
	...
	return {
		...
		addClass : function(E) {
			...
			Ext.each(E, function(G) {
						F.dom.className += (!F.hasClass(G) && G ? " " + G : "")
					});
			return F
		},
		radioClass : function(E) {
			Ext.each(this.dom.parentNode.childNodes, function(F) {
						if (F.nodeType == 1) {
							Ext.fly(F, "_internal").removeClass(E)
						}
					});
		...
		getStyle : function() {
			return s && s.getComputedStyle ? function(I) {
				var H = this.dom, E, G, F;
				if (H == document) {
					return null
				}
				I = v(I);
				F = (E = H.style[I]) ? E : (G = s.getComputedStyle(H, ""))
						? G[I]
						: null;
				if (Ext.isWebKit && F == "rgba(0, 0, 0, 0)") {
					F = "transparent"
				}
				return F
			} : function(I) {
		...
		addStyles : function(F, E) {
			var G = 0;
			Ext.each(F.match(/\w/g), function(H) {
						if (H = parseInt(this.getStyle(E[H]), 10)) {
							G += r.abs(H)
						}
					}, this);
			return G
...
Ext.Element.addMethods({
	scrollTo : function(b, d, a) {
		var e = /top/i.test(b), h = "scroll" + (e ? "Left" : "Top"), c = this, g = c.dom;
		if (!a || !c.anim) {
			g[h] = d
		} else {
			c.anim({
...
Ext.Container = Ext.extend(Ext.BoxComponent, {
	...
	doLayout : function(g, e) {
		var j = this.rendered;
		forceLayout = e || this.forceLayout;
		...
		if (g !== true && this.items) {
			...
			for (var b = 0, a = d.length; b < a; b++) {
				var h = d[b];
				if (h.doLayout) {
					h.doLayout(false, forceLayout)
				}
			}
		}
		if (j) {
			this.onLayout(g, forceLayout)
		}
		this.hasLayout = true;
...
Ext.layout.CardLayout = Ext.extend(Ext.layout.FitLayout, {
			...
			setActiveItem : function(b) {
				...
				if (this.activeItem != b) {
					...
					var a = b.doLayout
							&& (this.layoutOnCardChange || !b.rendered);
					this.activeItem = b;
					b.show();
					this.layout();
					if (a) {
	...
	onLayout : function(d, H) {
		...
		if (!this.rendered) {
			...
			for (var A = 0, B = v.length; A < B; A++) {
				...
				if (!E.rendered) {
					E.render(H, A);
					E.getDomPositionEl().addClass("x-border-panel")
				}
...
Ext.layout.FormLayout = Ext.extend(Ext.layout.AnchorLayout, {
	...
	renderItem : function(e, a, d) {
		if (e && (e.isFormField || e.fieldLabel) && e.inputType != "hidden") {
			...
			if (this.trackLabels && !this.isHide(e)) {
				if (e.hidden) {
					this.onFieldHide(e)
				}
				e.on({
	...
	isValidParent : function(b, a) {
		return a && this.container.getEl().contains(b.getDomPositionEl())
	}
});
...
Ext.layout.TableLayout = Ext.extend(Ext.layout.ContainerLayout, {
	...
	renderItem : function(e, a, d) {
		if (e && !e.rendered) {
			...
			if (e && !this.isValidParent(e, d)) {
				...
				b.insertBefore(e.getDomPositionEl().dom, null);
	...
	isValidParent : function(b, a) {
		return b.getDomPositionEl().up("table", 5).dom.parentNode === (a.dom || a)
	}
});
...
Ext.layout.VBoxLayout = Ext.extend(Ext.layout.BoxLayout, {
	...
	onLayout : function(e, E) {
		...
		var g = this.getItems(e), j, m, q, r = this.getTargetSize(E), k = r.width
				- E.getPadding("lr"), y = r.height - E.getPadding("tb")
				- this.scrollOffset, s = this.padding.left, n = this.padding.top, i = this.pack == "start", v = [
...
Ext.Panel = Ext.extend(Ext.Container, {
		...
		if (this.buttons) {
			...
			Ext.each(a, function(b) {
						if (b.render) {
							...
							if (b.xtype) {
								this.buttons.push(Ext.create(b, "button"))
							} else {
								this.addButton(b)
							}
						}
	...
	syncHeight : function() {
		var b = this.toolbarHeight, c = this.body, a = this.lastSize.height;
...
(function							() {
	...
	beforeDestroy : function() {
		...
		Ext.Element.uncache(this.ft, this.mc, this.header, this.tbar,
				this.bbar, this.footer, this.body, this.bwrap);
		if (this.tools) {
...
Ext.extend(Ext.data.Store, Ext.util.Observable, {
			...
			execute : function(d, a, b) {
				...
				var c = true;
				if (d === "read") {
					Ext.applyIf(b.params, this.baseParams);
...
Ext.data.DataWriter.prototype = {
	...
	toHash : function(e) {
		...
		if (e.phantom) {
			if (e.fields.containsKey(this.meta.idProperty)
					&& Ext.isEmpty(e.data[this.meta.idProperty])) {
				delete c[this.meta.idProperty]
			}
		} else {
...
Ext.extend(Ext.data.JsonWriter, Ext.data.DataWriter, {
			...
			render : function(c, a, d, b) {
				if (this.encode === true) {
					d[this.meta.root] = Ext.encode(b)
				} else {
					d.jsonData = {};
					d.jsonData[this.meta.root] = b
				}
			},
...
Ext.extend(Ext.data.JsonReader, Ext.data.DataReader, {
	...
	extractData : function(h, a) {
		...
		if (Ext.isArray(h)) {
			...
			if (a === true) {
				var j = this.recordType;
				for (var d = 0; d < h.length; d++) {
					var b = h[d];
					var e = new j(this.extractValues(b, l, k), this.getId(b));
					e.json = b;
					c.push(e)
				}
			} else {
				for (var d = 0; d < h.length; d++) {
					c.push(this.extractValues(h[d], l, k))
				}
			}
		}
		return c
...
Ext.extend(Ext.data.XmlWriter, Ext.data.DataWriter, {
	...
	render : function(c, a, d, b) {
		d.xmlData = this.tpl.applyTemplate({
					version : this.xmlVersion,
					encoding : this.xmlEncoding,
					record : this.meta.record,
					root : this.root,
					records : (Ext.isArray(a)) ? b : [b]
				})
	},
	toXml : function(b) {
		var a = [];
	...
	createRecord : function(a) {
		return this.toXml(this.toHash(a))
	},
	updateRecord : function(a) {
		return this.toXml(this.toHash(a))
	},
...
Ext.data.XmlReader = function(a, b) {
	a = a || {};
	a.idProperty = a.idProperty || a.idPath;
...
Ext.extend(Ext.data.XmlReader, Ext.data.DataReader, {
	...
	extractData : function(j, a) {
		var k = this.recordType, b = [], h = k.prototype.fields, m = h.items, l = h.length;
		if (a === true) {
			for (var d = 0, g = j.length; d < g; d++) {
				var c = j[d], e = new k(this.extractValues(c, m, l), this
								.getId(c));
				e.node = c;
				b.push(e)
			}
			...
			for (var d = 0, g = j.length; d < g; d++) {
				b.push(this.extractValues(j[d], m, l))
			}
		}
		return b
	},
...
Ext.data.GroupingStore = Ext.extend(Ext.data.Store, {
	...
	getGroupFieldValues : function() {
		...
		for (var i = 0; i < this.getCount(); i++) {
			...
			if (groupFieldValues.length == 0) {
				groupFieldValues.push(currentGroupFieldValue)
			} else if (groupFieldValues.indexOf(currentGroupFieldValue) == -1) {
				groupFieldValues.push(currentGroupFieldValue)
...
Ext.MessageBox = function() {
	var t, b, p, s, h, l, r, a, m, o, j, g, q, u, n, i = "", d = "";
	var c = function(w) {
		if (t.isVisible()) {
			t.hide();
			v();
	...
	var k = function(w) {
		...
		for (var x in q) {
			if (!Ext.isFunction(q[x])) {
				if (w[x]) {
					q[x].show();
					q[x].setText(Ext.isString(w[x])
							? w[x]
		...
		getDialog : function(w) {
			if (!t) {
				t = new Ext.Window({
					...
					minimizable : false,
					maximizable : false,
					// manager:Emin.getDesktop().getManager(),
					stateful : false,
					modal : true,
					shim : true,
					buttonAlign : "center",
				...
				q.ok = t.addButton(x.ok, c.createCallback("ok"));
				q.yes = t.addButton(x.yes, c.createCallback("yes"));
				q.no = t.addButton(x.no, c.createCallback("no"));
				q.cancel = t.addButton(x.cancel, c.createCallback("cancel"));
...
Ext.ListView = Ext.extend(Ext.DataView, {
	...
	initComponent : function() {
		...
		for (var g = 0; g < m; g++) {
			var n = Ext.apply({}, l[g]);
			if (!n.tpl) {
				n.tpl = new Ext.XTemplate("{" + n.dataIndex + "}")
			} else {
				if (Ext.isString(n.tpl)) {
					n.tpl = new Ext.XTemplate(n.tpl)
				}
			}
			n.align = n.align || "left";
			if (Ext.isNumber(n.width)) {
				n.width *= 100;
				h += n.width;
				k++
			}
...
Ext.ListView.ColumnResizer = Ext.extend(Ext.util.Observable, {
	...
	onEnd : function(l) {
		...
		this.proxy.remove();
		var h = this.hdIndex, m = this.view, g = m.columns, k = g.length, n = this.view.innerHd
				.getWidth(), d = this.minPct * 100;
		pct = Math.ceil((j * m.maxWidth) / n), diff = g[h].width - pct, each = Math
				.floor(diff / (k - 1 - h)), mod = diff - (each * (k - 1 - h));
		for (var c = h + 1; c < k; c++) {
			var b = g[c].width + each, a = Math.max(d, b);
			if (b != a) {
				mod += b - a
			}
			g[c].width = a
...
Ext.TabPanel = Ext.extend(Ext.Panel, {
	...
	onRemove : function(b) {
		Ext.TabPanel.superclass.onRemove.call(this, b);
...
Ext.Button = Ext.extend(Ext.BoxComponent, {
	...
	initButtonEl : function(b, e) {
		this.el = b;
		if (this.id) {
			var g = this.el.dom, h = Ext.Element.cache;
			delete h[g.id];
			g.id = this.el.id = this.id;
			h[g.id] = this.el
		}
		if (this.icon) {
			e.setStyle("background-image", "url(" + this.icon + ")")
		}
		if (this.tabIndex !== undefined) {
			e.dom.tabIndex = this.tabIndex
		}
		if (this.tooltip) {
...
Ext.QuickTip = Ext.extend(Ext.ToolTip, {
			...
			getTipCfg : function(d) {
				...
				if (this.interceptTitles && b.title) {
					c = b.title;
					b.qtip = c;
...
Ext.tree.TreePanel = Ext.extend(Ext.Panel, {
			...
			initComponent : function() {
				...
				this.addEvents("append", "remove", "movenode", "insert",
						...
						"click", "checkchange", "beforedblclick", "dblclick",
						"contextmenu", "beforechildrenrendered", "startdrag",
						"enddrag", "dragdrop", "beforenodedrop", "nodedrop",
						"nodedragover");
...
Ext.extend(Ext.tree.TreeLoader, Ext.util.Observable, {
			...
			createNode : function(attr) {
				...
				if (attr.nodeType) {
					...
					/*
					 * return attr.leaf ? new Ext.tree.TreeNode(attr) : new
					 * Ext.tree.AsyncTreeNode(attr)
					 */
...
Ext.form.ComboBox = Ext.extend(Ext.form.TriggerField, {
	...
	onKeyUp : function(b) {
		...
		if (this.editable !== false && a !== b.SPACE
...
Ext.form.HtmlEditor = Ext.extend(Ext.form.Field, {
	...
	syncValue : function() {
		if (this.initialized) {
			var d = this.getDoc();
			var c = d.body.parentNode.innerHTML ;
			
			if (Ext.isWebKit) {
				var b = this.getEditorBody().getAttribute("style");
	...
	pushValue : function() {
		if (this.initialized) {
			...
			if (this.fireEvent("beforepush", this, a) !== false) {
				this.doc.body.parentNode.innerHTML=""
				this.doc.body.parentNode.innerHTML=a;
	...
	adjustFont : function(b) {
		var c = b.getItemId() == "increasefontsize" ? 1 : -1;
...
Ext.form.VTypes = function() {
	var c = /^[a-zA-Z_]+$/, d = /^[a-zA-Z0-9_]+$/, b = /^(\w+)([\-+.][\w]+)*@(\w[\-\w]*\.){1,5}([A-Za-z]){2,4}$/, a = /(((^https?)|(^ftp)):\/\/([\-\w]+\.)+\w{2,3}(\/[%\-\w]+(\.\w{2,})?)*(([\w\-\.\?\\\/+@&#;`~=%!]*)(\.\w{2,})?)*\/?)/i;
	return {
		email : function(e) {
			return b.test(e)
		},
...
Ext.extend(Ext.grid.GridView, Ext.util.Observable, {
	...
	getColumnStyle : function(colIndex, isHeader) {
		var colModel = this.cm, colConfig = colModel.config, style = isHeader
				? ''
				: colConfig[colIndex].css || '', align = colConfig[colIndex].align;

		if (Ext.isChrome) {
			style += String.format("width: {0};", parseInt(this
							.getColumnWidth(colIndex))
							- 2 + 'px');
		} else {
			style += String
					.format("width: {0};", this.getColumnWidth(colIndex));
		}
...
Ext.grid.ColumnModel = Ext.extend(Ext.util.Observable, {
			/**
			 * @cfg {Number} defaultWidth (optional) The width of columns which
			 *      have no <tt>{@link #width}</tt> specified (defaults to
			 *      <tt>100</tt>). This property shall preferably be
			 *      configured through the <tt><b>{@link #defaults}</b></tt>
			 *      config property.
			 */
			defaultWidth : 100,

			/**
			 * @cfg {Boolean} defaultSortable (optional) Default sortable of
			 *      columns which have no sortable specified (defaults to
			...
			/**
			 * @cfg {Array} columns An Array of object literals. The config
			 *      options defined by <b>{@link Ext.grid.Column}</b> are the
			 *      options which may appear in the object literal for each
			 *      individual column definition.
			 */

			/**
			 * @cfg {Object} defaults Object literal which will be used to apply
			 *      {@link Ext.grid.Column} configuration options to all
			 *      <tt><b>{@link #columns}</b></tt>. Configuration options
			 *      specified with individual {@link Ext.grid.Column column}
			 *      configs will supersede these
			 *      <tt><b>{@link #defaults}</b></tt>.
			 */

			constructor : function(config) {
				/**
				 * An Array of {@link Ext.grid.Column Column definition} objects
				 * representing the configuration of this ColumnModel. See
				 * {@link Ext.grid.Column} for the configuration properties that
				 * may be specified.
				 * 
				 * @property config
				 * @type Array
				...
				this.addEvents(
						/**
						 * @event widthchange Fires when the width of a column
						 *        is programmaticially changed using
						 *        <code>{@link #setColumnWidth}</code>. Note
						 *        internal resizing suppresses the event from
						 *        firing. See also {@link Ext.grid.GridPanel}.<code>{@link #columnresize}</code>.
						 * @param {ColumnModel}
						 *            this
						 * @param {Number}
						...
						/**
						 * @event headerchange Fires when the text of a header
						 *        changes.
						 * @param {ColumnModel}
						 *            this
						 * @param {Number}
						...
						/**
						 * @event hiddenchange Fires when a column is hidden or
						 *        "unhidden".
						 * @param {ColumnModel}
						 *            this
						 * @param {Number}
						...
						/**
						 * @event columnmoved Fires when a column is moved.
						 * @param {ColumnModel}
						 *            this
						 * @param {Number}
						 *            oldIndex
						...
						/**
						 * @event configchange Fires when the configuration is
						 *        changed
						 * @param {ColumnModel}
						 *            this
						 */
			...
			/**
			 * <p>
			 * Reconfigures this column model according to the passed Array of
			 * column definition objects. For a description of the individual
			 * properties of a column definition object, see the <a
			 * href="#Ext.grid.ColumnModel-configs">Config Options</a>.
			 * </p>
			 * <p>
			 * Causes the {@link #configchange} event to be fired. A
			 * {@link Ext.grid.GridPanel GridPanel} using this ColumnModel will
			 * listen for this event and refresh its UI automatically.
			 * </p>
			 * 
			 * @param {Array}
			 *            config Array of Column definition objects.
			 * @param {Boolean}
			 *            initial Specify <tt>true</tt> to bypass cleanup
			 *            which deletes the <tt>totalWidth</tt> and destroys
			 *            existing editors.
			 */
			setConfig : function(config, initial) {
				...
				if (!initial) { // cleanup
					...
					for (i = 0, len = this.config.length; i < len; i++) {
						...
						if (c.setEditor) {
							// check here, in case we have a special column like
							// a CheckboxSelectionModel
							c.setEditor(null);
						}
					}
				}
			 ...
			 * 
			 * @param {Number}
			 *            oldIndex The index of the column to move.
			 * @param {Number}
			 *            newIndex The position at which to reinsert the
			 *            coolumn.
			 */
			 ...
			 * 
			 * @param {Boolean}
			 *            visibleOnly Optional. Pass as true to only include
			 *            visible columns.
			 * @return {Number}
			 */
			...
			/**
			 * Returns the column configs that return true by the passed
			 * function that is called with (columnConfig, index)
			 * 
			 * <pre><code>
			 ...
			 * 
			 * @param {Function}
			 *            fn A function which, when passed a
			 *            {@link Ext.grid.Column Column} object, must return
			 *            <code>true</code> if the column is to be included in
			 *            the returned Array.
			 * @param {Object}
			 *            scope (optional) The scope (<code>this</code>
			 *            reference) in which the function is executed. Defaults
			 *            to this ColumnModel.
			 * @return {Array} result
			 */
			...
			/**
			 * Returns the rendering (formatting) function defined for the
			 * column.
			 * 
			 * @param {Number}
			 *            col The column index.
			 * @return {Function} The function used to render the cell. See
			 *         {@link #setRenderer}.
			 */
			...
			/**
			 * Sets the rendering (formatting) function for a column. See
			 * {@link Ext.util.Format} for some default formatting functions.
			 * 
			 * @param {Number}
			 *            col The column index
			 * @param {Function}
			 *            fn The function to use to process the cell's raw data
			 *            to return HTML markup for the grid view. The render
			 ...
			 *            </p>
			 *            </li>
			 *            <li><b>metadata</b> : Object
			 *            <p class="sub-desc">
			 *            An object in which you may set the following
			 *            attributes:
			 *            <ul>
			 *            <li><b>css</b> : String
			 *            <p class="sub-desc">
			 ...
			 *            data container element <i>within</i> the table cell
			 *            (e.g. 'style="color:red;"').
			 *            </p>
			 *            </li>
			 *            </ul>
			 ...
			 *            </p>
			 *            </li>
			 *            <li><b>store</b> : Ext.data.Store
			 *            <p class="sub-desc">
			 *            The {@link Ext.data.Store} object from which the
			 ...
			 *            width The new width
			 * @param {Boolean}
			 *            suppressEvent True to suppress firing the
			 *            <code>{@link #widthchange}</code> event. Defaults to
			 *            false.
			 */
			...
			/**
			 * Finds the index of the first matching column for the given
			 * dataIndex.
			 * 
			 * @param {String}
			...
			/**
			 * Returns <tt>true</tt> if the column is
			 * <code>{@link Ext.grid.Column#hidden hidden}</code>,
			...
			isHidden : function(colIndex) {
				return !!this.config[colIndex].hidden; // ensure returns
														// boolean
			},

			/**
			 * Returns <tt>true</tt> if the column is
			 * <code>{@link Ext.grid.Column#fixed fixed}</code>,
			 * <tt>false</tt> otherwise.
			 * 
			 * @param {Number}
			 *            colIndex The column index
			 ...
			 * Destroys and dereferences all Columns.
			 */
			destroy : function() {
				if(this.config!=undefined){
					...
					for (; i < length; i++) {
						this.config[i].destroy(); // Column's destroy encapsulates
													// all cleanup.
					}
				}
			...
			/**
			 * @private Setup any saved state for the column, ensures that
...
/*
 * Ext.grid.Column.prototype = { isColumn : true, renderer : function(a) { if
 * (Ext.isString(a) && a.length < 1) { return "&#160;" } return a },
 * 
 * getEditor : function(a) { return this.editable !== false ? this.editor : null },
 * getCellEditor : function(b) { var a = this.getEditor(b); if (a) { if
 * (!a.startEdit) { if (!a.gridEditor) { a.gridEditor = new
 * Ext.grid.GridEditor(a) } return a.gridEditor } else { if (a.startEdit) {
 * return a } } } return null } };
 */
Ext.grid.Column = Ext.extend(Ext.util.Observable, {
			/**
			 * @cfg {Boolean} editable Optional. Defaults to <tt>true</tt>,
			 *      enabling the configured <tt>{@link #editor}</tt>. Set to
			 *      <tt>false</tt> to initially disable editing on this
			 *      column. The initial configuration may be dynamically altered
			 *      using {@link Ext.grid.ColumnModel}.{@link Ext.grid.ColumnModel#setEditable setEditable()}.
			 */
			/**
			 * @cfg {String} id Optional. A name which identifies this column
			 *      (defaults to the column's initial ordinal position.) The
			 *      <tt>id</tt> is used to create a CSS <b>class</b> name
			 *      which is applied to all table cells (including headers) in
			 *      that column (in this context the <tt>id</tt> does not need
			 ...
			 * </pre>
			 * 
			 * Header cells will also receive this class name, but will
			 *      also have the class
			 * 
			 * <pre>
			 * x - grid3 - hd
			 ...
			 * </pre>
			 * 
			 * The {@link Ext.grid.GridPanel#autoExpandColumn} grid config
			 *      option references the column via this unique identifier.
			 */
			/**
			 * @cfg {String} header Optional. The header text to be used as
			 *      innerHTML (html tags are accepted) to display in the Grid
			 *      view. <b>Note</b>: to have a clickable header with no text
			 *      displayed use <tt>'&amp;#160;'</tt>.
			 */
			/**
			 * @cfg {Boolean} groupable Optional. If the grid is being rendered
			 *      by an {@link Ext.grid.GroupingView}, this option may be
			 *      used to disable the header menu item to group by the column
			 *      selected. Defaults to <tt>true</tt>, which enables the
			 *      header menu group option. Set to <tt>false</tt> to disable
			 *      (but still show) the group option in the header menu for the
			 *      column. See also <code>{@link #groupName}</code>.
			 */
			/**
			 * @cfg {String} groupName Optional. If the grid is being rendered
			 *      by an {@link Ext.grid.GroupingView}, this option may be
			 *      used to specify the text with which to prefix the group
			 *      field value in the group header line. See also
			 *      {@link #groupRenderer} and {@link Ext.grid.GroupingView}.{@link Ext.grid.GroupingView#showGroupName showGroupName}.
			 */
			/**
			 * @cfg {Function} groupRenderer
			 *      <p>
			 *      Optional. If the grid is being rendered by an
			 *      {@link Ext.grid.GroupingView}, this option may be used to
			 *      specify the function used to format the grouping field value
			 *      for display in the group {@link #groupName header}. If a
			 *      <tt><b>groupRenderer</b></tt> is not specified, the
			 *      configured <tt><b>{@link #renderer}</b></tt> will be
			 *      called; if a <tt><b>{@link #renderer}</b></tt> is also
			 *      not specified the new value of the group field will be used.
			 *      </p>
			 *      <p>
			 *      The called function (either the
			 *      <tt><b>groupRenderer</b></tt> or
			 *      <tt><b>{@link #renderer}</b></tt>) will be passed the
			 *      following parameters: <div class="mdetail-params">
			 *      <ul>
			 *      <li><b>v</b> : Object
			 *      <p class="sub-desc">
			 ...
			 *      <p class="sub-desc">
			 *      The Record providing the data for the row which caused group
			 *      change.
			 *      </p>
			 *      </li>
			 *      <li><b>rowIndex</b> : Number
			 ...
			 */
			/**
			 * @cfg {String} emptyGroupText Optional. If the grid is being
			 *      rendered by an {@link Ext.grid.GroupingView}, this option
			 *      may be used to specify the text to display when there is an
			 *      empty group value. Defaults to the
			 ...
			 *      {@link Ext.data.Store}'s {@link Ext.data.Record} definition
			 *      from which to draw the column's value.
			 *      </p>
			 */
			/**
			 * @cfg {Number} width Optional. The initial width in pixels of the
			 *      column. The width of each column can also be affected if any
			 *      of the following are configured: <div
			 ...
			 *      <div class="sub-desc">
			 *      <p>
			 *      By specifying <tt>forceFit:true</tt>,
			 *      {@link #fixed non-fixed width} columns will be
			 *      re-proportioned (based on the relative initial widths) to
			 *      fill the width of the grid so that no horizontal scrollbar
			 *      is shown.
			 *      </p>
			 *      </div></li>
			 ...
			 *      <br>
			 *      <p>
			 *      <b>Note</b>: when the width of each column is determined, a
			 *      space on the right side is reserved for the vertical
			 *      scrollbar. The {@link Ext.grid.GridView}.<tt>{@link Ext.grid.GridView#scrollOffset scrollOffset}</tt>
			 *      can be modified to reduce or eliminate the reserved offset.
			 *      </p>
			 */
			/**
			 * @cfg {Boolean} sortable Optional. <tt>true</tt> if sorting is
			 *      to be allowed on this column. Defaults to the value of the
			 *      <code>{@link Ext.grid.ColumnModel#defaultSortable}</code>
			 *      property. Whether local/remote sorting is used is specified
			 *      in <code>{@link Ext.data.Store#remoteSort}</code>.
			 */
			/**
			 * @cfg {Boolean} fixed Optional. <tt>true</tt> if the column
			 *      width cannot be changed. Defaults to <tt>false</tt>.
			 */
			/**
			 * @cfg {Boolean} resizable Optional. <tt>false</tt> to disable
			 *      column resizing. Defaults to <tt>true</tt>.
			 */
			/**
			 * @cfg {Boolean} menuDisabled Optional. <tt>true</tt> to disable
			 *      the column menu. Defaults to <tt>false</tt>.
			 */
			/**
			 * @cfg {Boolean} hidden Optional. <tt>true</tt> to initially hide
			 *      this column. Defaults to <tt>false</tt>. A hidden column
			 *      {@link Ext.grid.GridPanel#enableColumnHide may be shown via the header row menu}.
			 *      If a column is never to be shown, simply do not include this
			 *      column in the Column Model at all.
			 */
			/**
			 * @cfg {String} tooltip Optional. A text string to use as the
			 *      column header's tooltip. If Quicktips are enabled, this
			 *      value will be used as the text of the quick tip, otherwise
			 *      it will be set as the header's HTML title attribute.
			 *      Defaults to ''.
			 */
			/**
			 * @cfg {Mixed} renderer
			 ...
			 *      <code>{@link #xtype}</code>
			 *      </p>
			 *      <p>
			 *      Optional. A renderer is an 'interceptor' method which can be
			 *      used transform data (value, appearance, etc.) before it is
			 *      rendered). This may be specified in either of three ways:
			 *      <div class="mdetail-params">
			 *      <ul>
			 *      <li>A renderer function used to return HTML markup for a
			 *      cell given the cell's data value.</li>
			 *      <li>A string which references a property name of the
			 *      {@link Ext.util.Format} class which provides a renderer
			 *      function.</li>
			 *      <li>An object specifying both the renderer function, and
			 *      its execution scope (<tt><b>this</b></tt> reference)
			 *      e.g.:
			 * 
			 ...
			 *      parameters, etc.), see
			 *      {@link Ext.grid.ColumnModel#setRenderer}. An example of
			 *      specifying renderer function inline:
			 *      </p>
			 * 
			 * <pre><code>
			 * var companyColumn = {
			 ...
			 */
			/**
			 * @cfg {String} xtype Optional. A String which references a
			 *      predefined {@link Ext.grid.Column} subclass type which is
			 *      preconfigured with an appropriate
			 *      <code>{@link #renderer}</code> to be easily configured
			 *      into a ColumnModel. The predefined {@link Ext.grid.Column}
			 *      subclass types are: <div class="mdetail-params">
			 *      <ul>
			 *      <li><b><tt>gridcolumn</tt></b> :
			 *      {@link Ext.grid.Column} (<b>Default</b>)
			 *      <p class="sub-desc">
			 *      </p>
			 *      </li>
			 *      <li><b><tt>booleancolumn</tt></b> :
			 *      {@link Ext.grid.BooleanColumn}
			 *      <p class="sub-desc">
			 *      </p>
			 *      </li>
			 *      <li><b><tt>numbercolumn</tt></b> :
			 *      {@link Ext.grid.NumberColumn}
			 *      <p class="sub-desc">
			 *      </p>
			 *      </li>
			 *      <li><b><tt>datecolumn</tt></b> :
			 *      {@link Ext.grid.DateColumn}
			 *      <p class="sub-desc">
			 *      </p>
			 *      </li>
			 *      <li><b><tt>templatecolumn</tt></b> :
			 ...
			 */
			/**
			 * @cfg {Object} scope Optional. The scope (<tt><b>this</b></tt>
			 *      reference) in which to execute the renderer. Defaults to the
			 *      Column configuration object.
			 */
			/**
			 * @cfg {String} align Optional. Set the CSS text-align property of
			 *      the column. Defaults to undefined.
			 */
			/**
			 * @cfg {String} css Optional. An inline style definition string
			 *      which is applied to all table cells in the column (excluding
			 *      headers). Defaults to undefined.
			 */
			/**
			 * @cfg {Boolean} hideable Optional. Specify as <tt>false</tt> to
			 *      prevent the user from hiding this column (defaults to true).
			 *      To disallow column hiding globally for all columns in the
			 *      grid, use {@link Ext.grid.GridPanel#enableColumnHide}
			 *      instead.
			 */
			/**
			 * @cfg {Ext.form.Field} editor Optional. The {@link Ext.form.Field}
			 *      to use when editing values in this column if editing is
			 *      supported by the grid. See <tt>{@link #editable}</tt>
			 *      also.
			 */

			/**
			 * @private
			 * @cfg {Boolean} isColumn Used by ColumnModel setConfig method to
			 *      avoid reprocessing a Column if <code>isColumn</code> is
			 *      not set ColumnModel will recreate a new Ext.grid.Column
			 *      Defaults to true.
			 */
			isColumn : true,
			...
			constructor : function(config) {
				...
				this.addEvents(
						/**
						 * @event click Fires when this Column is clicked.
						 * @param {Column}
						 *            this
						 * @param {Grid}
						...
						'click',
						/**
						 * @event contextmenu Fires when this Column is right
						 *        clicked.
						 * @param {Column}
						 *            this
						 * @param {Grid}
						...
						'contextmenu',
						/**
						 * @event dblclick Fires when this Column is double
						 *        clicked.
						 * @param {Column}
						 *            this
						 * @param {Grid}
						...
						'dblclick',
						/**
						 * @event mousedown Fires when this Column receives a
						 *        mousedown event.
						 * @param {Column}
						 *            this
						 * @param {Grid}
			 ...
			 *          processEvent method. Returns the event handler's status
			 *          to allow cancelling of GridView's bubbling process.
			 */
			...
			/**
			 * @private Clean up. Remove any Editor. Remove any listeners.
			 */
			destroy : function() {
				if (this.setEditor) {
					this.setEditor(null);
			...
			/**
			 * Optional. A function which returns displayable data when passed
			 * the following parameters: <div class="mdetail-params">
			 * <ul>
			 * <li><b>value</b> : Object
			 * <p class="sub-desc">
			 * The data value for the cell.
			 ...
			 * container element <i>within</i> the table cell (e.g.
			 * 'style="color:red;"').
			 * </p>
			 * </li>
			 * </ul>
			 ...
			 * <li><b>store</b> : Ext.data.Store
			 * <p class="sub-desc">
			 * The {@link Ext.data.Store} object from which the Record was
			 * extracted.
			 * </p>
			 * </li>
			 * </ul>
			 ...
			 * 
			 * @param {Ext.Editor/Ext.form.Field}
			 ...
			 * that was created to wrap the {@link Ext.form.Field Field} used to
			 * edit the cell.
			 * 
			 * @param {Number}
			 *            rowIndex The row index
...
Ext.grid.TemplateColumn = Ext.extend(Ext.grid.Column, {
	constructor : function(a) {
		...
		var b = Ext.isObject(this.tpl) ? this.tpl : new Ext.XTemplate(this.tpl);
...
Ext.grid.EditorGridPanel = Ext.extend(Ext.grid.GridPanel, {
			...
			startEditing : function(i, c) {
				...
				if (this.colModel.isCellEditable(c, i)) {
					...
					var h = this.colModel.getDataIndex(c);
					var g = {
						grid : this,
						record : d,
						field : h,
...
Ext.grid.GroupingView = Ext.extend(Ext.grid.GridView, {
	...
	doRender : function(d, j, t, a, s, v) {
		...
		var k = "width:" + this.getTotalWidth() + ";", u = this.grid
				.getGridEl().id, h = this.cm.config[q], b = h.groupRenderer
				|| h.renderer, w = this.showGroupName
				? (h.groupName || h.header) + ": "
				: "", B = [], n, x, y, p;
		for (x = 0, y = j.length; x < y; x++) {
			var m = a + x, o = j[x], e = o.data[C];
			z = this.getGroup(e, o, b, m, q, t);
			if (!n || n.group != z) {
				p = this.constructId(e, u, C, q);
	...
	getGroupId : function(a) {
		...
		return this.constructId(a, this.grid.getGridEl().id, b, this.cm
						.findColumnIndex(b))
	},
	constructId : function(d, c, g, a) {
		var b = this.cm.config[a], e = b.groupRenderer || b.renderer, h = (this.groupMode == "value")
				? d
				: this.getGroup(d, {
							data : {}
						}, e, 0, a, this.ds);
		return c + "-gp-" + g + "-" + Ext.util.Format.htmlEncode(h)
	},
...
Ext.extend(Ext.ux.ComboBoxTree, Ext.form.ComboBox, {
	...
	getValue : function() {
		...
		// return typeof this.node != 'undefined' ? this.node.id : '';
	},
...
Ext.extend(Ext.grid.RowExpander, Ext.util.Observable, {
	...
	getRowClass : function(record, rowIndex, p, ds) {
		...
		if (record.data.status != null) {
			s = record.data.status != 1 ? "rowDisable" : ""
		}

		return this.state[record.id]
				? 'x-grid3-row-expanded ' + s
				: 'x-grid3-row-collapsed ' + s;
	},
...
Ext.ux.form.DateRange = Ext.extend(Ext.form.TriggerField, {
	...
	initEvents : function() {
		...
		this.mon(this.el, 'keypress', function(e, f) {
					if (e.getKey() == Ext.EventObject.ENTER) { // catch Return.
						...
						if (this.isDateRange()) {
							this.handler(ds[0], ds[1]);
						} else {
							this.handler(ds, false);
						}
					}
				}, this, {
	...
	setValue : function(date) {
		...
		if (date == '') {
			return Ext.form.DateField.superclass.setRawValue.call(this, value);
		}
		...
		if (Ext.isDate(date)) {
			...
			if (date.length == 1) {
				...
				value = this.formatDate(date[0]) + this.delimiter
						+ this.formatDate(date[1]);
			}
		} else if (this.isDateRange(date)) {
			value = date; // pass thru
		} else {
			value = this.formatDate(Date.parse(date)); // try a parse (last
														// shot)
		}
		// alert(value)
	...
	onTriggerClick : function() {
		...
		var dateToday = Date.parse('today'), dateBasePast = Date
				.parse('today-10years');

		var cdates = $this.getDates(), cdateS = (cdates.length > 0)
				? cdates[0]
				: dateToday, cdateE = (cdates.length > 1) ? cdates[1] : cdateS;

		if (cdateS == null || cdateS == "") {
			cdateS = dateToday
		}
		if (cdateE == null || cdateE == "") {
			cdateE = cdateS
		}
		...
		if (this.menu === undefined) {
			...
			if (false === Ext.isEmpty(this.presetDates)
					&& false === Ext.isEmpty(this.presets)) {
				menu.add('-');
			}
					...
					case 'allDatesAfter' :
						dateMenu = new Ext.menu.DateMenu({
									...
									handler : function(p, d) {
										var date = d;
										if (key == 'allDatesBefore') {
											date = [dateBasePast, d];
										}
										if (key == 'allDatesAfter') {
											date = [d, dateToday];
										}
										$this.setValue(date);
						...
						switch (key) {
							case 'specificDate' :
								dateMenu.picker.setValue(cdateE);
								break;
							case 'allDatesBefore' :
								dateMenu.picker.setValue(cdateS);
								break;
							case 'allDatesAfter' :
								dateMenu.picker.setValue(cdateE);
								break;
						...
						break;
					case 'dateRange' :
						var instanciateDatePickerPanel = function(config,
								pickerCfg) {
							var $this = this;
						...
						p1 = instanciateDatePickerPanel({
									role : 'start',
									title : '开始日期'
								}, {
									maxDate : dateToday
								});
						p2 = instanciateDatePickerPanel({
									role : 'end',
									title : '结束日期'
								}, {
									maxDate : dateToday
...
Date.prototype.getWeekOfYear = function(firstDayOfWeek) {
	...
	var daynum = ((Date.UTC(y, m, d, 0, 0, 0) - Date.UTC(y, 0, 1, 0, 0, 0)) / 86400000)
			+ 1;
	...
	var _ = $P.Operators = {
		...
		list : function(p, d, c) {
			...
			return (p instanceof Array ? _.each(_.product(p.slice(0, -1), _
									.ignore(d)), p.slice(-1), _.ignore(c)) : _
...
(function() {
	...
	Date.Translator = {
		...
		month : function(s) {
			return function() {
				this.month = ((s.length == 3)
						? Date.getMonthNumberFromName(s)
						: (Number(s) - 1));
			};
		},
		year : function(s) {
			return function() {
				var n = Number(s);
				this.year = ((s.length > 2)
						? n
						: (n + (((n + 2000) < Date.CultureInfo.twoDigitYearMax)
								? 2000
								: 1900)));
			};
		},
		...
		finishExact : function(x) {
			...
			this.hour = (this.meridian == "p" && this.hour < 13) ? this.hour
					+ 12 : this.hour;
		...
		finish : function(x) {
			...
			if (expression) {
				var gap, mod, orient;
				orient = ((this.orient == "past" || this.operator == "subtract")
						? -1
						: 1);
				if (this.weekday) {
					this.unit = "day";
				...
				if (this.meridian && this.hour) {
					this.hour = (this.hour < 13 && this.meridian == "p")
							? this.hour + 12
							: this.hour;
				}
	...
	g.MMM = g.MMMM = _.cache(_.process(g
	...
	g.format = _
			.process(
					_
						...
						return _.process(_.each.apply(null, rules),
								t.finishExact);
					});
	var _F = {};
	var _get = function(f) {
...
Ext.ux.Spinner = Ext.extend(Ext.util.Observable, {
	...
	fixPrecision : function(value) {
		var nan = isNaN(value);
		if (!this.field.allowDecimals || this.field.decimalPrecision == -1
				|| nan || !value) {
			return nan ? '' : value;
		}
...
Ext.Slider.Tip = Ext.extend(Ext.Tip, {
			...
			/**
			 * @private Called whenever a dragstart or drag event is received on
			 *          the associated Thumb. Aligns the Tip with the Thumb's
			 *          new position.
			...
			/**
			 * Used to create the text that appears in the Tip's body. By
			 * default this just returns the value of the Slider Thumb that the
			 * Tip is attached to. Override to customize.
			 * 
			 * @param {Ext.slider.Thumb}
			 *            thumb The Thumb that the Tip is attached to
...
Ext.grid.ActionColumn = Ext.extend(Ext.grid.Column, {
	/**
	 * @cfg {String} icon The URL of an image to display as the clickable
	 *      element in the column. Optional - defaults to
	 *      <code>{@link Ext#BLANK_IMAGE_URL Ext.BLANK_IMAGE_URL}</code>.
	 */
	/**
	 * @cfg {String} iconCls A CSS class to apply to the icon image. To
	 *      determine the class dynamically, configure the Column with a
	 *      <code>{@link #getClass}</code> function.
	 */
	/**
	 * @cfg {Function} handler A function called when the icon is clicked. The
	 *      handler is passed the following parameters:<div
	 *      class="mdetail-params">
	 *      <ul>
	 *      <li><code>grid</code> : GridPanel<div class="sub-desc">The
	 *      owning GridPanel.</div></li>
	 *      <li><code>rowIndex</code> : Number<div class="sub-desc">The row
	 *      index clicked on.</div></li>
	 *      <li><code>colIndex</code> : Number<div class="sub-desc">The
	 *      column index clicked on.</div></li>
	 *      <li><code>item</code> : Object<div class="sub-desc">The clicked
	 *      item (or this Column if multiple {@link #items} were not
	 *      configured).</div></li>
	 ...
	 *      which the <code>{@link #handler}</code> and
	 *      <code>{@link #getClass}</code> fuctions are executed. Defaults to
	 *      this Column.
	 */
	/**
	 * @cfg {String} tooltip A tooltip message to be displayed on hover.
	 *      {@link Ext.QuickTips#init Ext.QuickTips} must have been initialized.
	 */
	/**
	 * @cfg {Boolean} stopSelection Defaults to <code>true</code>. Prevent
	 *      grid <i>row</i> selection upon mousedown.
	 */
	/**
	 * @cfg {Function} getClass A function which returns the CSS class to apply
	 *      to the icon image. The function is passed the following parameters:<div
	 *      class="mdetail-params">
	 *      <ul>
	 *      <li><b>v</b> : Object
	 *      <p class="sub-desc">
	 ...
	 *      element <i>within</i> the table cell (e.g. 'style="color:red;"').
	 *      </p>
	 *      </li>
	 *      </ul>
	 *      </p>
	 ...
	 */
	/**
	 * @cfg {Array} items An Array which may contain multiple icon definitions,
	 *      each element of which may contain: <div class="mdetail-params">
	 *      <ul>
	 *      <li><code>icon</code> : String<div class="sub-desc">The url of
	 *      an image to display as the clickable element in the column.</div></li>
	 *      <li><code>iconCls</code> : String<div class="sub-desc">A CSS
	 *      class to apply to the icon image. To determine the class
	 *      dynamically, configure the item with a <code>getClass</code>
	 *      function.</div></li>
	 *      <li><code>getClass</code> : Function<div class="sub-desc">A
	 *      function which returns the CSS class to apply to the icon image. The
	 ...
	 *      </ul>
	 *      </div></li>
	 *      <li><code>handler</code> : Function<div class="sub-desc">A
	 *      function called when the icon is clicked.</div></li>
	 *      <li><code>scope</code> : Scope<div class="sub-desc">The scope (<code><b>this</b></code>
	 *      reference) in which the <code>handler</code> and
	 *      <code>getClass</code> functions are executed. Fallback defaults
	 *      are this Column's configured scope, then this Column.</div></li>
	 *      <li><code>tooltip</code> : String<div class="sub-desc">A tooltip
	 *      message to be displayed on hover.
	...
	/**
	 * @cfg {String} altText The alt text to use for the image element. Defaults
	 *      to <tt>''</tt>.
	 */
	altText : '',
			...
			for (i = 0; i < l; i++) {
				...
				v += '<img alt="'
						...
						+ (Ext.isFunction(item.getClass)
								? item.getClass.apply(item.scope || this.scope
												|| this, arguments)
								: '')
						+ '"'
						+ ((item.tooltip)
								? ' ext:qtip="' + item.tooltip + '"'
								: '') + ' />';
			}
			return v;
		};
	...
	/**
	 * @private Process and refire events routed from the GridView's
	 *          processEvent method. Also fires any configured click handlers.
	 *          By default, cancels the mousedown event to prevent selection.
	 *          Returns the event handler's status to allow cancelling of
	 *          GridView's bubbling process.
	 */
	processEvent : function(name, e, grid, rowIndex, colIndex) {
		...
		return Ext.grid.ActionColumn.superclass.processEvent.apply(this,
				arguments);
	}
});
Ext.ux.grid.ActionColumn = Ext.extend(Ext.grid.ActionColumn, {
	constructor : function(cfg) {
		...
		me.renderer = function(v, meta,r) {
			...
			for (i = 0; i < l; i++) {
				item = items[i];
				if(me.validRecord && item.valid(r)==false){
					continue;
				}
				var cls = Ext.isFunction(item.getClass) ? item.getClass.apply(
						item.scope || this.scope || this, arguments) : '';
				var tooltip = Ext.isFunction(item.tooltip)
						? item.tooltip.apply(item.scope || this.scope || this,
								arguments)
						: item.tooltip;
				tooltip = tooltip != "" ? (' ext:qtip="' + tooltip + '"') : ""
				
				var text = Ext.isFunction(item.text)?item.text.apply(item.scope || this.scope || this,
								arguments):item.text;
				var lineheight = ""
				if(cfg.lineheight){
					lineheight="style='line-height:"+cfg.lineheight+";height:"+cfg.lineheight+"'";
				}
				v += '<div class="ux-action-col-item '
						+ (item.iconCls || '')
						+ ' x-action-col-'
						+ String(i)
						+ ' '
						+ (text ? 'ux-action-col-text ' : '')
						+ cls
						+ '"'
						+ tooltip
						+ ' '+lineheight+'>'
	...
	// // class name x-action-col-{n}
	// me.renderer = function(v, meta) {
	// // Allow a configured renderer to create initial value (And set the other
	// values in the "metadata" argument!)
	// v = Ext.isFunction(cfg.renderer) ? cfg.renderer.apply(this,
	// arguments)||'' : '';
	//
	// meta.css += ' x-action-col-cell';
	// for (i = 0; i < l; i++) {
	// item = items[i];
	// v += '<img alt="' + (item.altText || me.altText) + '" src="' + (item.icon
	// || Ext.BLANK_IMAGE_URL) +
	// '" class="x-action-col-icon x-action-col-' + String(i) + ' ' +
	// (item.iconCls || '') +
	// ' ' + (Ext.isFunction(item.getClass) ?
...
HTMLEditor = Ext.extend(Ext.form.HtmlEditor, {
				...
				listeners:{
					click:function(d,i,n,e){
						...
						if(e.target.name=="remove"){
							...
							var element = document.createElement("img");
                            element.src = record.data.url;
                            element.style="width:120px;height:120px"
                            if (Ext.isIE) {
                                editor.insertAtCursor(element.outerHTML);
                            } else {
                                var selection = editor.win.getSelection();
							...
							items : [
								{
									...
									filePostName:"file",
									uploadUrl : basePath+'weixin-file!uploadFile',
    ...
    createToolbar : function(editor) {
        ...
        this.tb.insertButton(16, {itemId : "picture",
				cls : "x-btn-icon",
				icon : "ext-3.0.3/resources/images/default/editor/picture.png",
				enableToggle : false,
				scope : this,
